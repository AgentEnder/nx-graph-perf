...
// nx is also loaded into this process for the report data; its daemon status
// must describe the same environment the measured runs had.
...
async function readReportData() {
  const module =
    resolveFromWorkspace("nx/dist/src/command-line/report/report") ??
    resolveFromWorkspace("nx/src/command-line/report/report");
...
function installInstrument(source) {
  const target =
    resolveFromWorkspace("nx/dist/src/utils/perf-logging.js") ??
    resolveFromWorkspace("nx/src/utils/perf-logging.js");
  if (!target) {
...
function renderReport(report) {
  ...
  if (report.projectGraphError)
    parts.push(md.blockQuote(`Project graph error: ${report.projectGraphError}`));
  parts.push(
  ...
  if (report.mismatchedNxVersions?.length) {
    parts.push(
      md.h3(
        ...
        md.table(
          /** @type {{ version: string, chain: string[] }[]} */ (report.mismatchedNxVersions),
          [
            { label: "version", field: "version" },
...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(opts.out, "graph-perf.json"),
